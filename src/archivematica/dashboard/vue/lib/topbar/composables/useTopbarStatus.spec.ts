...
describe('useTopbarStatus', () => {
  ...
  it('updates counts and connection state on success', async () => {
    ...
    const { useTopbarStatus } = await import('@/topbar/composables/useTopbarStatus')
    const { state, pollOnce } = useTopbarStatus()
  ...
  it('delays loading indicator to avoid flicker', async () => {
    let resolveFetch: ((value: StatusResponse | null) => void) | undefined
    vi.mocked(getStatus).mockImplementation(
      () =>
        new Promise((resolve) => {
          resolveFetch = resolve
        }) as Promise<StatusResponse | null>,
    )
