...
function buildLabel(name: string, properties?: Record<string, unknown>): string {
  const displayString = properties?.['display_string']
  return typeof displayString === 'string' ? `${name} (${displayString})` : name
}
...
function buildNodes(response: FilesystemBrowseResponse, parentPath: string): MetadataTreeNode[] {
  ...
  return response.entries.map((entry): MetadataTreeNode => {
    ...
    return {
      id: path,
      label: buildLabel(decodedName, response.properties?.[entry]),
      path,
